...
class ABAW3BaseModel(nn.Module):
    def __init__(self, bacbone, backbone_freeze=None, backbone_freeze_bn=True, aux_feat=None, transf_nhead=8,
        ...
        # exit()
        # self.backbone2, self.num_feats2 =  get_backbone('resnet18', backbone_freeze, backbone_freeze_bn, aux_feat)
        # num_feat_spatial = self.num_feats[-1] + self.num_feats2[-1]
    ...
    def forward(self, x, is_train=True, indexes=None):
        ...
        # backbone_feats2 = self.backbone2(feat)
        # backbone_feats = torch.cat([backbone_feats, backbone_feats2], dim=1)
        # print(backbone_feats.shape)
        # exit()

        # print(feat.shape)
        # print(backbone_feats['feat'].shape)
        # exit()
...
class ABAW3Model(LightningModule):
    ...
    def __init__(self, do_mixup=False, use_aux=0.2):
        ...
        # print(cfg.MODEL.BACKBONE_PRETRAINED)
        # exit()
        if cfg.MODEL.BACKBONE_PRETRAINED != 'none':
            # import sys
            model=torch.load(cfg.MODEL.BACKBONE_PRETRAINED)
            ...
            self.model.temporal_head.tranf.requires_grad_(True)
            # Freeze temporal head fc
            self.model.temporal_head.fc_head.requires_grad_(True)
    ...
    def update_metric(self, out, y, is_train=True):
        ...
        if is_train:
            ...
            # print("y={}".format(y[:2]))
            # print("val_metric={}".format(self.val_metric(out[:2], y[:2])))
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict({'train_metric': self.train_metric, 'train_loss': loss}, on_step=False, on_epoch=True, prog_bar=True,
    ...
    def get_metrics_criterion(self):
        if self.task == 'AU':
            ...
            #                         class_weights=self.cls_weights, alpha=cfg.OPTIM.FOCAL_ALPHA, gamma=cfg.OPTIM.FOCAL_GAMMA)
            ...
            self.train_metric = F1Score(task='multilabel', threshold=self.threshold, num_labels=self.num_outputs, num_classes=2, average='macro')
            self.val_metric = F1Score(task='multilabel', threshold=self.threshold, num_labels=self.num_outputs, num_classes=2, average='macro')
            # self.train_metric = F1Score(task='binary', threshold=self.threshold, num_classes=self.num_outputs, average='macro')
            # self.val_metric = F1Score(task='binary',threshold=self.threshold, num_classes=self.num_outputs, average='macro')
