...
class ABAW3Dataset(data.Dataset):
    def __init__(self, split='Train', transforms=None):
        ...
        if split != 'Test':
            # data_dict = \
            #     np.load('/data0/zza/cvpr23_competition/au_detection_task/data_process/train_set_for_au.npy').item()[
            #         self.split]
            data_dict = \
    ...
    def __getitem__(self, index):
        ...
        for idx in range(cur_seq_sample.shape[0]):
            ...
            if self.task == 'AU':
                sample['AU'].append([float(x) for x in cur_seq_sample[idx, 1:-1]])
            else:   
...
class ABAW3DataModule(LightningDataModule):
    ...
    def train_dataloader(self, shufflex=None):
        ...
        if shufflex is not None:
            shuffle = shufflex
        # print("dataset_len={}".format(len(self.train_dataset)))
