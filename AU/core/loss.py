...
def CELogitLoss(y_hat, y, scale_factor=1, num_classes=8, label_smoothing=0., class_weights=None):
    y_hat_fl = torch.reshape(y_hat, (-1, num_classes))
    y_fl = torch.reshape(y, (-1, num_classes))  # Class indices
...
def CEFocalLoss(y_hat, y, scale_factor=1, num_classes=8, label_smoothing=0., class_weights=None, alpha=0.25, gamma=2.,
    ...
    y_hat_fl = torch.reshape(y_hat, (-1, num_classes))
    y_fl = torch.reshape(y, (-1,num_classes)) *1.0  # Class indices

    # print("y_hat_f1:{}".format(y_hat_fl.size()))
    # print("y_f1:{}".format(y_fl.size()))

    ce_loss = F.cross_entropy(y_hat_fl, y_fl, label_smoothing=label_smoothing, reduction='none')
    target_one_hot = F.one_hot(y_fl.to(torch.int64), num_classes=num_classes)
    print("target_one_hot size={}".format(target_one_hot.size()))
    ...
    if distillation_loss:
        ...
        print("target_one_hot_smooth size={}".format(target_one_hot_smooth.size()))
        print("input size={}".format(F.log_softmax(y_hat_fl, dim=1).size()))
